...
- 文档生成日期：2026-07-15
- 当前分支：`master`
- 当前 commit：`73bd07effa932cc467d39b73c224ba3c1119d5e4`（阶段 17 开始时审计基线；本文包含待提交的阶段 17 变更）
- 项目技术栈：原生 HTML/CSS/JavaScript ES Module + FastAPI + PyMySQL + MySQL 8.0.28
- 数据库名称：`frieren_cloth_shop_db`
- 后端端口：`8050`
- 前端端口：`5900`
- 当前自动测试结果：阶段 17 最终结果 `npm.cmd test` 220/220 通过；`tests.product_tags_batch_unit` 4/4 通过
...
| 命令 | 结果 |
|---|---|
| `npm.cmd test` | 阶段 17 最终结果 220/220 通过，0 失败、0 跳过、0 todo |
