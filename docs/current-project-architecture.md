...
- 后端端口：`8050`
- 前端端口：`5900`
- 当前自动测试结果：`npm.cmd test` 共 130 项，130 项通过；`src/main.js`、`src/account-store.js`、`src/sku-utils.js`、`src/product-ordering.js`、`src/ranking.js` 语法检查和后端 Python 编译检查均通过
- 文档基线：审计起始 commit 为 `d8e298f`（`fix: 修复商品 SKU 选择状态同步`）；本文档包含当前工作区待提交的阶段 6 商品收藏与购物车交互调整

> 本文档描述当前代码快照。自动测试以纯函数行为和源码结构契约为主；既有退款轮次已完成专用订单验证。阶段 4 已完成商品介绍的本地 MySQL、HTTP 与浏览器验收；阶段 5 已完成 SKU 选择纯函数与浏览器验收；阶段 6 已完成商品级收藏迁移、三类操作语义分流和桌面端/390px 浏览器验收，但未执行真实购物车或订单写入；未执行的项目会明确标注。
...
| `localStorage` | 前台本地账号资料、商品级收藏、购物车快照、购物车勾选项；旧后台商品/mock 订单 | 收藏以 `blue-song-favorites` 商品级规范记录为准并自动迁移旧 SKU 记录；购物车商品以数据库为业务源但 UI 使用回读快照；后台 API 失败仍有 mock 回退 |
...
| 命令 | 结果 |
|---|---|
| `npm.cmd test` | 130/130 通过，0 失败、0 跳过、0 TODO |
...
- 覆盖销量排名、可售优先排序、地址迁移/本地存储、商品级收藏规范化/去重/幂等迁移/切换、购物车金额、注册校验、SKU 笛卡尔积、新增商品 SKU 默认库存 50、矩阵重建时保留人工库存、已有商品缺失组合仍默认 0、颜色尺码选择和不可售组合禁用、有效/失效显式 SKU 恢复、唯一可售 SKU 自动选择、多 SKU 未选、商品级缓存同步和 ES Module 缓存版本契约，以及后台图片唯一入口、商品介绍迁移/接口/安全展示/编辑弹窗契约。
...
130 项中相当一部分是 `readFileSync(...).includes(...)` 或正则形式的源码结构断言；它们能锁定契约，但不是浏览器或 API 端到端测试。
...
- **浏览器测试**：测试套件未内置浏览器框架；阶段 4 完成商品介绍浏览器回归，阶段 5 完成 SKU 选择同步回归，阶段 6 完成商品级收藏、下架商品收藏、收藏夹删除、购物车/立即购买弹窗和 390px 响应式验收。旧 SKU 收藏的合并、损坏记录隔离与幂等写回由纯函数和内存 localStorage 测试覆盖；浏览器控制面未直接注入旧 localStorage。未执行真实购物车/订单写入和网络请求拦截。
...
- 没有手动设置任意商品图片为主图的接口。
- 收藏图标红色、购物车图标黄色、数量徽章、收藏夹卡片详情增强和数据库收藏均未实现，属于后续阶段。
- 没有纳入常驻套件的自动化 HTTP、真实数据库、并发事务或浏览器端到端测试；当前真实数据库验证覆盖既有退款场景和阶段 4 商品介绍场景，阶段 6 浏览器验收只写入并恢复 localStorage 收藏，未写入购物车或订单数据。
