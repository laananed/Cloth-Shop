...
6. `06_商品描述增量迁移.sql`
7. `07_订单买家备注增量迁移.sql`
8. `08_购物车选中项备注下单增量迁移.sql`
9. `09_商品多标签增量迁移.sql`
10. `10_管理员操作日志增量迁移.sql`

`01`～`05` 用于全新测试库的完整初始化，其中 `04` 会清空并重建固定演示数据，禁止在已有业务数据的数据库执行。`06`～`10` 是兼容增量迁移，均显式选择数据库并可重复执行；已有环境只执行尚未应用的增量。`MANIFEST_SHA256.txt` 记录 01～10 当前字节数与 SHA256。
...
- 测试订单插入触发器生成初始状态日志后，`04` 会在写固定演示历史前清理这些临时日志，避免主键冲突。
- `06` 显式选择目标数据库，保证可按编号独立执行。
