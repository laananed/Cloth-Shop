...
- 文档生成日期：2026-07-15
- 当前分支：`Cloth-Shop-logs`
- 当前 commit：`06663ca6de4d32c4894fb5008735ced63b845830`（本次任务开始时审计基线）
- 项目技术栈：原生 HTML/CSS/JavaScript ES Module + FastAPI + PyMySQL + MySQL 8.0.28
- 数据库名称：`frieren_cloth_shop_db`
- 后端端口：`8050`
- 前端端口：`5900`
- 当前自动测试结果：`npm.cmd test` 共 188 项，188 项通过；`src/main.js`、`src/category-utils.js`、`src/image-lightbox.js`、`src/account-store.js`、`src/content.js`、`src/sku-utils.js`、`src/product-ordering.js`、`src/ranking.js` 语法检查和后端 Python 编译检查均通过
- 文档基线：审计起始 commit 为 `06663ca6de4d32c4894fb5008735ced63b845830`（`feat: 完善商品分类管理闭环`）；本文档包含当前工作区待提交的阶段 16 管理员操作日志闭环

> 本文档描述当前代码快照。自动测试以纯函数行为和源码结构契约为主；既有退款轮次已完成专用订单验证。阶段 4—13 的既有验收保持不变；阶段 16 为全部现有管理员写操作增加同事务成功审计和回滚后失败审计，提供受保护的日志查询 API 与只读后台面板，并完成真实 MySQL 迁移、API、桌面端与 390px 浏览器验收；未执行的项目会明确标注。

## 1. 项目概述

Cloth-Shop 是一个服装商城与进销存管理课程设计。前台承担商品浏览、分类筛选、SKU 选择、收藏、购物车、地址、下单、支付和订单记录；后台承担管理员认证、分类、商品/SKU/图片/库存、订单发货、退款审核、销量统计和只读操作审计。FastAPI 提供 HTTP 接口、鉴权、文件上传与事务编排，PyMySQL 连接 MySQL；MySQL 保存分类、商品、库存、订单、支付、日志和销量，并通过视图、存储过程、触发器维护主要业务一致性。
...
│  ├─ [09 编号保留，当前分支无文件]   # 并行商品标签工作线预留
│  └─ 10_管理员操作日志增量迁移.sql   # operation_log 字段与三组查询索引的幂等增量
├─ tests/site.test.js                 # 188 项 Node.js 行为与源码结构测试
...
| 新增商品 | `data-admin-product-form` | `buildSkuMatrix()`、`createAdminProductToApi()`；选择现有有效 `category_id`，提交最多 1000 字普通文本介绍，新生成 SKU 默认库存 50 | `POST /products` | `category`、`product.description`、`product_sku`、`inventory`、`product_sales_stat`、`product_image` |
...
后台导航当前有订单查看、商品管理、分类管理、新增商品、销量统计、操作日志六项。商品接口失败且不是 401/403 时，前端仍可能回退到 `localStorage` 的 mock 商品，这是数据库权威数据源方向下需要收敛的兼容逻辑。操作日志面板错误只更新本面板反馈；401/403 继续复用统一登录失效清理。
...
| POST | `/products` | multipart 新增商品；优先校验现有有效 `category_id`，兼容旧 `category_name` 仅查找且不会自动创建/恢复分类 | 管理员 Bearer 令牌 | `category`、`product`、`product_sku`、`inventory`、`product_sales_stat`、`product_image` |
...
9. `sql语句/09_商品多标签增量迁移.sql`：编号固定保留给并行商品标签工作线；当前日志分支未包含该文件。
10. `sql语句/10_管理员操作日志增量迁移.sql`：幂等增加 `target_type`、`target_id`、`action_result`、`detail_json`、`request_id` 与动作/目标/结果时间索引；保留旧字段、旧索引和旧日志。

从零初始化时必须按以上顺序执行；合并完整分支后顺序固定为 `08 → 09 → 10`。已存在数据库只需按编号执行尚未应用的增量；`04` 当前是固定规模演示数据（16 商品、32 SKU、8 订单等），不是大量销售压力数据。
...
- **后台接口权限**：后台商品、SKU、图片、库存、订单、统计和操作日志接口调用 `require_admin_user()`，要求 Bearer token，并重新查询用户的管理员/删除状态。
...
| 命令 | 结果 |
|---|---|
| `npm.cmd test` | 188/188 通过，0 失败、0 跳过、0 TODO |
...
- 直接执行 `content.js`、`category-utils.js`、`ranking.js`、`product-ordering.js`、`account-store.js`、`sku-utils.js`、`image-lightbox.js` 的纯函数行为。
...
188 项中相当一部分是 `readFileSync(...).includes(...)` 或正则形式的源码结构断言；它们能锁定契约，但不是浏览器或 API 端到端测试。

### 尚未覆盖或本轮未执行

- **自动化 HTTP 冒烟测试**：尚未纳入常驻测试套件；阶段 4 使用本地临时脚本覆盖商品介绍状态矩阵并在结束后恢复数据。
- **真实 API 测试范围**：商品介绍与阶段 9 立即购买既有覆盖保持不变；阶段 10 覆盖购物车备注；阶段 13 覆盖分类权限、CRUD/恢复、统计、冲突、幂等、商品移动和新增商品分类校验；阶段 16 覆盖日志权限、成功/失败落库、筛选、排序、分页、边界和敏感信息；尚未覆盖令牌过期、数据库故障注入和所有后台写接口的完整运行时状态矩阵，图片/SKU/订单/退款审计由源码契约测试覆盖。
- **真实数据库测试**：阶段 4 与阶段 9 既有覆盖保持不变；阶段 10 已验证订单备注迁移和事务；阶段 13 复用既有结构并验证分类/商品关系与精确数据恢复；阶段 16 已将日志迁移连续执行两次并验证保留旧数据、成功同事务、失败回滚和精确清理；未执行故障注入回滚或并发压力测试。
- **浏览器测试**：测试套件未内置浏览器框架；阶段 4—12 的既有浏览器回归保持不变。阶段 13 已覆盖分类闭环；阶段 16 已覆盖日志桌面/390px 布局、筛选、分页、详情、空态、401 清理及受影响后台模块和前台商品回归，验收标签页控制台 0 error。
- **退款回归**：已覆盖请求模型字段、退款路由不读取 SKU 字段、不调用购买校验、订单归属锁、允许状态、状态更新、提交/回滚、业务错误保留以及前端订单级请求体。

## 13. 本地启动流程

1. 启动 MySQL 8.0，并确认将使用 `frieren_cloth_shop_db`。
2. 按第 8.4 节顺序执行 `sql语句/` 下 `01`～`10`；`09` 是并行工作线保留编号，当前分支没有该文件时直接执行现有脚本。`04` 会清空并重建测试数据，只能用于测试库；已有数据库只执行尚未应用的增量脚本。
...
| `backend/app/main.py` | 单文件包含 46 个路由、模型、上传、鉴权、SQL 和事务 | Pydantic 模型、前端请求、SQL 对象、状态机、提交/回滚、错误码和上传清理 |
...
| `sql语句/01-10` | 表、视图、过程、触发器、测试数据、账号初始化、商品介绍、订单备注和操作日志增量按序耦合；`09` 为当前分支保留编号 | 增量迁移、外键/索引、最终覆盖视图、过程兼容、测试数据与 README 执行顺序 |
...
以上结论来自当前代码、SQL、自动测试及定向运行验证；阶段 10 购物车下单、阶段 13 商品分类和阶段 16 管理员操作日志均已完成真实数据库、API、浏览器与数据清理验收，阶段 11 商品状态印章与阶段 12 商品大图预览已完成真实 API 数据下的桌面与 390px 浏览器验收，阶段 4—9 与退款的既有验收记录保持不变，其他浏览器交互仍未完整验收。
...
- **SQL 初始化与业务过程**：阶段 9 已真实执行 `07` 两次，阶段 10 已真实执行 `08` 两次，阶段 16 已真实执行 `10` 两次并验证幂等与旧日志保留；`04` 的全量演示数据重建本轮未执行。
...
- 标签管理/批量标签仍未实现。本轮也未扩展支付方式、支付网关、订单合并、管理员备注编辑或跨设备购物车备注草稿。
